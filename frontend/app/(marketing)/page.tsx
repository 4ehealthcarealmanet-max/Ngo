...
import { 
  ShieldCheck, Globe, Lightbulb, Heart, Building2, Users, 
  BookOpen, HeartHandshake, Stethoscope, Handshake, Code2, HeartPulse 
} from 'lucide-react';
interface NGO {
  id: number;
  name: string;
    ...
    <div className="min-h-screen bg-white font-sans text-brand-slate pt-20">
        ...
        <div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7 text-left space-y-8 pt-4">
            ...
            <div className="max-w-2xl rounded-[20px] border border-slate-200 bg-white p-2 shadow-xl shadow-slate-100 flex flex-col md:flex-row items-center gap-2">
              <div className="flex flex-1 items-center px-4 py-2 w-full border-r border-slate-100">
                ...
                <input type="text" placeholder="Your City" value={cityInput} onChange={(e) => setCityInput(e.target.value)} className="bg-transparent outline-none w-full text-base font-semibold text-slate-900 placeholder:text-[#ADB5BD]" />
              </div>
              <div className="flex flex-1 items-center px-4 py-2 w-full">
                ...
                <input type="text" placeholder="Search Service..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="bg-transparent outline-none w-full text-base font-semibold text-slate-900 placeholder:text-[#ADB5BD]" />
              </div>
              <button onClick={handleSearch} className="rounded-xl bg-brand-blue px-10 py-3 text-base font-bold text-white hover:bg-blue-700 transition shadow-lg w-full md:w-auto">Search</button>
            </div>
            ...
            <div className="pt-4 flex items-start justify-start gap-12">
              {[
                ...
                <div key={point.title} className="group flex flex-col items-center text-center gap-2 w-28 cursor-pointer">
                  ...
                  <span className="text-[12px] font-bold text-slate-700 leading-tight group-hover:text-brand-blue transition-colors">{point.title}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative flex flex-col items-center justify-center pt-4 group">
            <div className="relative rounded-[40px] overflow-hidden shadow-2xl border-[16px] border-slate-50 max-w-[460px] animate-float transition-all duration-700 ease-in-out hover:scale-110 cursor-pointer">
              <img src="/ngocamp.png" alt="NGO Camp" className="w-full h-[480px] object-cover transition-transform duration-700 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/20 to-transparent pointer-events-none"></div>
            </div>
            <div className="absolute -top-6 -left-8 bg-white/95 backdrop-blur-sm p-4 rounded-2xl shadow-xl shadow-blue-900/5 border border-slate-100 flex items-center gap-4 animate-float z-20" style={{ animationDelay: '0.3s' }}>
              <div className="relative h-12 w-12 rounded-full bg-[#E8FBF4] text-[#12B886] flex items-center justify-center border border-[#D3F9EB]">
                ...
                <svg className="relative w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" /></svg>
              </div>
              <div>
                <p className="text-[10px] font-bold text-[#12B886] uppercase tracking-[0.2em] flex items-center gap-1.5 mb-0.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#12B886] animate-pulse"></span>Live Now
                </p>
      ...
      <section className="relative py-20 bg-white overflow-hidden border-y border-slate-100">
        <div className="absolute inset-0 z-0 pointer-events-none">
          <img src="/bgimage.png" alt="Background" className="w-full h-full object-cover opacity-0.1" style={{ mixBlendMode: 'multiply' }} />
        </div>
        ...
        <div className="mx-auto max-w-7xl px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            <div className="lg:w-1/3 text-left">
              <span className="text-brand-blue font-bold tracking-[0.15em] uppercase text-[10px] bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 shadow-sm">NGO Ecosystem</span>
              <h2 className="text-4xl font-black mt-5 text-slate-950 tracking-tighter leading-tight">Core <span className="text-brand-blue">Pillars</span></h2>
              <p className="text-slate-500 mt-4 text-[16px] font-medium leading-relaxed">Our three core modules designed to make healthcare accessible and digital.</p>
            </div>
  ...
  <div className="mx-auto max-w-7xl">
    ...
    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
      <div className="space-y-3">
        <span className="inline-flex items-center gap-2 text-blue-500 font-bold tracking-[0.2em] uppercase text-[10px] shadow-sm bg-red-50 px-4 py-2 rounded-full border border-red-100 w-fit">
          🩸 Blood Donor Program
        </span>
        <h2 className="text-3xl md:text-4xl font-black text-slate-950 tracking-tight">
          Become a <span className="text-brand-blue">Blood Donor</span>
        </h2>
        <p className="text-slate-500 text-sm font-medium max-w-xl">
          Register once. When a hospital nearby urgently needs your blood group, we alert you instantly.
        </p>
      </div>
      <Link href="/donor/register">
        <button className="group flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-black px-7 py-3.5 rounded-2xl transition-all duration-300 shadow-lg shadow-blue-100 hover:shadow-blue-200 hover:-translate-y-0.5 text-sm whitespace-nowrap">
    ...
    {/* 3 Step Cards - Full Width Now */}
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-5">
      {[
        {
          step: "01",
          title: "Register",
          desc: "Fill a quick form with your name, blood group, and phone number. Takes 2 minutes.",
        ...
        },
        {
          step: "02",
          title: "Get Alerted",
          desc: "When a hospital needs your blood group urgently nearby, you receive an instant alert on your phone.",
        ...
        },
        {
          step: "03",
          title: "Save a Life",
          desc: "Accept the request, visit the hospital, and donate. One small decision can change everything.",
        ...
        <div
          key={item.step}
          className="bg-white rounded-[1.75rem] border border-slate-100 p-8 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 group relative overflow-hidden"
        >
          {/* Step number background watermark - z-0 so it stays behind */}
          <span className="absolute bottom-2 right-5 text-8xl font-black text-slate-100 select-none pointer-events-none leading-none z-0">
            {item.step}
          </span>

          {/* All content - z-10 so it stays in front */}
          <div className="relative z-10">
            {/* Icon */}
            <div className={`h-14 w-14 rounded-2xl ${item.color} border flex items-center justify-center mb-6`}>
              {item.icon}
            </div>

            {/* Step label */}
            <span className={`inline-block text-[9px] font-black tracking-[0.25em] uppercase px-3 py-1 rounded-full text-white ${item.badge} mb-3`}>
              Step {item.step}
            </span>
    ...
    {/* Bottom Note - small & subtle */}
    <div className="flex items-center justify-center gap-2 mt-2">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 text-emerald-500 shrink-0">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
      </svg>
      <p className="text-xs text-slate-400 font-medium">
        Your number is <span className="font-black text-slate-600">encrypted & secure</span> — only used for blood emergencies. You can opt out anytime.
      </p>
    </div>
      ...
      {/* ================= WORKSHOPS ================= */}
      <section id="events" className="py-20 bg-white border-t border-slate-100">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
            <div className="space-y-3">
              <span className="text-brand-blue font-bold tracking-[0.2em] uppercase text-[10px] shadow-sm bg-blue-50/50 px-4 py-2 rounded-full border border-blue-100/50 w-fit block">Community Outreach</span>
              <h2 className="text-3xl md:text-4xl font-black text-slate-950 tracking-tight">Upcoming Workshops</h2>
              <p className="text-slate-500 text-sm font-medium max-w-2xl">Live sessions fetched from the backend API for real-time updates.</p>
            </div>
          </div>
          {ecosystemData.workshops.length === 0 ? (
            <div className="bg-slate-50 border border-slate-100 rounded-3xl p-8 text-slate-600 font-medium">No workshops found yet. Add some from Django Admin and refresh.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {ecosystemData.workshops.map((workshop) => (
                <div key={workshop.id} className="group bg-white border border-slate-100 rounded-[2rem] p-6 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all">
                  <div className="flex items-start justify-between gap-4">
                    <h4 className="text-lg font-black text-slate-950 group-hover:text-brand-blue transition-colors">{workshop.title}</h4>
                    <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest border shrink-0 ${workshop.is_open ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-100 text-slate-400 border-slate-200"}`}>{workshop.is_open ? "Open" : "Full"}</span>
                  </div>
                  <p className="mt-2 text-sm text-slate-500 font-medium line-clamp-2 overflow-hidden [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical]">{workshop.description}</p>
                  <button type="button" onClick={() => router.push(`/workshops/${workshop.id}`)} className="mt-2 text-xs font-black text-brand-blue hover:underline underline-offset-4 w-fit">Read More...</button>
                  <div className="mt-4 space-y-2 text-sm text-slate-600 font-medium">
                    <p><span className="text-slate-400 font-bold">Expert:</span> {workshop.expert_name}</p>
                    <p><span className="text-slate-400 font-bold">Date:</span> {workshop.date}</p>
                  </div>
                  <button disabled={!workshop.is_open} onClick={() => openRegisterModal(workshop)} className="mt-6 w-full py-4 rounded-2xl font-black text-sm transition-colors shadow-lg shadow-blue-200 disabled:opacity-60 disabled:cursor-not-allowed bg-blue-600 text-white hover:bg-blue-700 disabled:hover:bg-blue-600">
      ...
      {/* ================= OUR PROGRAMMES ================= */}
      <section id="programs" className="py-32 bg-brand-light/40 relative overflow-hidden">
        <div className="absolute inset-0 z-0 pointer-events-none"><img src="/bgimage.png" alt="Background" className="w-full h-full object-cover opacity-1.5" style={{ mixBlendMode: 'multiply' }} /></div>
        <div className="absolute inset-0 bg-gradient-to-b from-white/80 via-transparent to-white/80 z-[1]"></div>
        <div className="absolute -top-40 -right-40 h-80 w-80 rounded-full bg-blue-50/70 border border-blue-100/50"></div>
        <div className="mx-auto max-w-7xl px-6 relative z-10">
          <div className="text-center mb-20 max-w-3xl mx-auto">
            <span className="text-brand-blue font-bold tracking-[0.2em] uppercase text-[10px] shadow-sm bg-blue-50/50 px-4 py-2 rounded-full border border-blue-100/50">Strategic Initiatives</span>
            <h2 className="text-4xl md:text-5xl font-black mt-6 text-slate-950 leading-tight camelcase tracking-tight">Our <span className="text-brand-blue">Programmes</span></h2>
            <p className="text-slate-500 mt-6 text-[18px] leading-relaxed font-medium">Driving sustainable change through targeted healthcare interventions and community-led development initiatives.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-12">
            {[
              ...
              <Link key={impact.title} href={`/programmes/${impact.title.toLowerCase().replace(/\s+/g, "-")}`} className="block">
                <div className="group cursor-pointer bg-white p-9 rounded-[28px] border border-slate-100 shadow-lg hover:shadow-2xl hover:bg-blue-600 hover:border-blue-600 hover:-translate-y-2.5 transition-all duration-500 animate-in fade-in-up ease-out h-full flex flex-col min-h-[340px]" style={{ animationDelay: `${index * 150}ms` }}>
                  ...
                  <h3 className="text-2xl font-black mb-4 text-slate-950 group-hover:text-white transition-all duration-300 min-h-[64px] leading-tight">{impact.title}</h3>
                  <p className="text-slate-600 leading-relaxed font-medium group-hover:text-white/80 transition-all duration-300 flex-1">{impact.text}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      {/* ================= NGO LIST ================= */}
      <section id="ngo" ref={resultsSectionRef} className="bg-brand-light py-24 px-6 scroll-mt-24">
        <div className="mx-auto max-w-7xl">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12">{[1, 2, 3].map((i) => (<div key={i} className="h-80 animate-pulse rounded-3xl bg-slate-200"></div>))}</div>
          ) : (
              ...
              {ngos.length > 0 ? ngos.map((ngo) => (
                <div key={ngo.id} className="group rounded-3xl bg-white p-10 shadow-sm border border-slate-100 hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                  ...
                  <p className="text-sm font-semibold text-slate-400 mb-8 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-brand-blue"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" /></svg>
                    {ngo.city}
                  </p>
                  <div className="flex items-center justify-between pt-8 border-t border-slate-50">
                    <span className="text-xs font-bold text-brand-blue bg-blue-50 px-4 py-2 rounded-xl">{ngo.service_type}</span>
                    <button type="button" onClick={() => router.push(`/ngos/${ngo.id}`)} className="text-sm font-bold text-slate-900 hover:text-brand-blue transition-colors group-hover:underline decoration-2 underline-offset-4">View Details →</button>
                  </div>
                </div>
              )) : (
...
<section id="about" className="py-32 bg-white relative overflow-hidden">
  <div className="absolute -top-40 -left-40 h-80 w-80 rounded-full bg-blue-50/50 blur-3xl pointer-events-none"></div>
  <div className="absolute -bottom-40 -right-40 h-80 w-80 rounded-full bg-blue-50/50 blur-3xl pointer-events-none"></div>
  
  <div className="mx-auto max-w-7xl px-6 relative z-10">
    
    {/* Top Badge */}
    <div className="text-center mb-20">
      <span className="text-brand-blue font-bold tracking-[0.2em] uppercase text-[10px] bg-blue-50/50 px-4 py-2 rounded-full border border-blue-100/50">About Us</span>
      <h2 className="text-4xl md:text-5xl font-black mt-6 text-slate-950 leading-tight tracking-tight">
        Who We Are & <span className="text-brand-blue">Why We Exist</span>
      </h2>
      <p className="text-slate-500 mt-6 text-[18px] leading-relaxed font-medium max-w-3xl mx-auto">
        Pathyatech is a healthcare technology platform on a mission to bridge the gap between underprivileged communities and quality medical care across India.
      </p>
    </div>
      ...
      <div className="space-y-8">
        <div className="space-y-4">
          ...
          <p className="text-slate-600 text-lg leading-relaxed font-medium">
            We believe healthcare is a fundamental right, not a privilege. Through technology, verified NGO partnerships, and community-driven programs, we are creating a transparent and accessible healthcare ecosystem for every Indian — regardless of their location or economic background.
          </p>
        </div>

        <div className="space-y-4">
          <h3 className="text-3xl font-black text-slate-950 tracking-tight">Our Vision</h3>
          <p className="text-slate-600 text-lg leading-relaxed font-medium">
            A future where no one is denied medical care due to distance, cost, or lack of information. We envision a fully connected rural healthcare network powered by verified NGOs, trained professionals, and digital tools.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-4">
         {[
  { icon: <ShieldCheck size={22} className="text-blue-600" />, bg: "bg-blue-50", title: "Trust", desc: "Verified NGOs & doctors only" },
  { icon: <Globe size={22} className="text-emerald-600" />, bg: "bg-emerald-50", title: "Accessibility", desc: "Rural-first approach" },
  { icon: <Lightbulb size={22} className="text-amber-600" />, bg: "bg-amber-50", title: "Innovation", desc: "Tech-driven healthcare" },
  { icon: <Heart size={22} className="text-rose-600" />, bg: "bg-rose-50", title: "Compassion", desc: "Community-led care" },
].map((val) => (
  <div key={val.title} className="bg-white border border-slate-100 rounded-2xl p-5 hover:border-blue-200 hover:shadow-md transition-all">
    <div className={`h-10 w-10 ${val.bg} rounded-xl flex items-center justify-center`}>
      {val.icon}
    </div>
    <h4 className="text-sm font-black text-slate-900 mt-3">{val.title}</h4>
    <p className="text-xs font-medium text-slate-500 mt-1">{val.desc}</p>
  </div>
))}
        </div>
      </div>
...
onError={(e) => {
        ...
        <div className="absolute -bottom-6 -left-6 bg-white rounded-2xl shadow-xl border border-slate-100 p-5 flex items-center gap-4">
          <div className="h-12 w-12 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-lg">P</div>
          <div>
            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Founded</p>
            <p className="text-sm font-black text-slate-900">Pathyatech · 2024</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-32">
      {[
  { number: "50+", label: "Active NGOs", icon: <Building2 size={28} className="text-blue-600" />, bg: "bg-blue-50", color: "bg-white border-blue-100" },
{ number: "1200+", label: "Verified Patients", icon: <Users size={28} className="text-emerald-600" />, bg: "bg-emerald-50", color: "bg-white border-emerald-100" },
{ number: "30+", label: "Live Workshops", icon: <BookOpen size={28} className="text-purple-600" />, bg: "bg-purple-50", color: "bg-white border-purple-100" },
{ number: "10K+", label: "Lives Impacted", icon: <HeartHandshake size={28} className="text-rose-600" />, bg: "bg-rose-50", color: "bg-white border-rose-100" },
      ].map((stat) => (
       <div key={stat.label} className={`${stat.color} border rounded-3xl p-8 text-center hover:shadow-xl hover:-translate-y-2 transition-all duration-300`}>
  <div className={`${stat.bg} h-14 w-14 rounded-2xl flex items-center justify-center mx-auto`}>
    {stat.icon}
  </div>
          <h3 className="text-4xl font-black text-slate-950 mt-4 tracking-tight">{stat.number}</h3>
    ...
    {/* CTA */}
    <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-[3rem] p-12 md:p-16 text-white text-center relative overflow-hidden">
      <div className="absolute top-0 right-0 h-full w-1/2 bg-white/5 -skew-x-12 translate-x-1/4"></div>
      <div className="relative z-10">
        <h3 className="text-4xl font-black tracking-tight mb-4">Join Our Mission 🚀</h3>
        <p className="text-blue-100 text-lg font-medium max-w-xl mx-auto mb-8">
          Whether you're an NGO, doctor, or volunteer — there's a place for you in the Pathyatech ecosystem.
        </p>
  ...
  onClick={() => {
    ...
    setTimeout(() => {
      document.getElementById('navbar-register-btn')?.click();
    }, 500);
  }}
  className="bg-white text-blue-700 px-10 py-4 rounded-2xl font-black text-lg hover:shadow-xl transition-all"
>
  Get Started →
</button>
          <a href="/#ngo" className="border-2 border-white/30 text-white px-10 py-4 rounded-2xl font-black text-lg hover:bg-white/10 transition-all">
            View NGOs
          </a>
        </div>
      </div>
